...
declare variable $TOKS-FIELD := (
  cts:punctuation(':'), cts:punctuation('='), $TOKS-INEQ, cts:word("EQ"), $TOKS-BANG);
(: for range query terms - inequality :)
declare variable $TOKS-INEQ := (
  cts:punctuation('>'), cts:punctuation('<'),
  cts:word("GT"), cts:word("LT"), cts:word("GE"), cts:word("LE"), cts:word("NE"));
declare variable $TOKS-INEQ-VALID := (
  $TOKS-INEQ, cts:punctuation('>='), cts:punctuation('<='), $TOKS-NE);
declare variable $TOKS-BANG := cts:punctuation('!');
declare variable $TOKS-NE := cts:punctuation('!=');
declare variable $TOKS-INFIX := ($TOKS-AND, $TOKS-OR, $TOKS-NEAR);
declare variable $TOKS-OP-JOIN := (cts:punctuation('/'));
declare variable $TOKS-AND := (cts:word('AND'), cts:word('and'), cts:punctuation('&amp;'));
declare variable $TOKS-OR := (cts:word('OR'), cts:word('or'), cts:punctuation('|'));
declare variable $TOKS-NEAR := (cts:word('NEAR'), cts:word('near'), cts:word('ONEAR'), cts:word('onear'));
declare variable $TOKS-PREFIX := ( cts:word('NOT'), cts:word('not'), cts:punctuation('-'), cts:punctuation('+'), cts:punctuation('~'));
...
declare private variable $TOK_BREAK :=
	( $TOKS-FIELD, $TOK-GROUP-START, $TOK-GROUP-END, $TOK-QUOTE, $TOK-SPACE );

declare private variable $op-map as map:map := map:map (
  <map:map xmlns:map="http://marklogic.com/xdmp/map">
   <map:entry><map:key>GT</map:key><map:value>&gt;</map:value></map:entry>
   <map:entry><map:key>GE</map:key><map:value>&gt;=</map:value></map:entry>
   <map:entry><map:key>LT</map:key><map:value>&lt;</map:value></map:entry>
   <map:entry><map:key>LE</map:key><map:value>&lt;=</map:value></map:entry>
   <map:entry><map:key>NE</map:key><map:value>!=</map:value></map:entry>
   <map:entry><map:key>EQ</map:key><map:value>=</map:value></map:entry>
  </map:map>
);
...
as xs:string?
{
  ...
  else if ($next = $TOK_BREAK) then ($tok, p:rewind(1))
  else string-join (($tok, $next,
      if (not($DEBUG)) then () else p:debug-state('word: next-until'),
      p:next-until (p:next(), $TOK_BREAK)),
    '')
};
...
as xs:string
{
  (: join complex ops, eg '<=' :)
  let $field-op :=
    if ($op = ($TOKS-INEQ, $TOKS-BANG) and concat($op, $next) = $TOKS-INEQ-VALID)
    then concat($op, $next)
    else if ($op eq $TOKS-BANG) then $TOKS-NE else $op
  let $mapped-op := map:get ($op-map, $field-op)
  return if ($mapped-op) then $mapped-op else $field-op
};
